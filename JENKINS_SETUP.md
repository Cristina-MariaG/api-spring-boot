...
> The PEM key is automatically generated by Terraform and copied to `~/.ssh/springboot-api.pem` by `setup-infra.sh`. Use `cat ~/.ssh/springboot-api.pem` to retrieve its content.
...
> ```bash
> ssh-keygen -p -m PEM -f ~/.ssh/springboot-api.pem
> ```
