...
**Requires:** [Terraform](https://developer.hashicorp.com/terraform/install), [Ansible](https://docs.ansible.com/ansible/latest/installation_guide/), and [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html) configured with an account that has EC2, S3, and IAM permissions (`aws configure`).
...
The Terraform state is stored remotely in S3 (versioning + AES-256 encryption) and locked via S3 native locking to prevent concurrent deployments — see [`aws/README.md`](aws/README.md) for a detailed explanation of this pattern.
