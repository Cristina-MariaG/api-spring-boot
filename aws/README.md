...
The PEM key is generated automatically. `setup-infra.sh` copies it to `~/.ssh/springboot-api.pem` and applies `chmod 600` — required because `chmod` does not work on `/mnt/c/` (NTFS via WSL).
